    ...
    <div>
      ...
      {invoices.length === 0 ? (
        ...
        <div className="rounded-2xl border-2 border-foreground overflow-hidden shadow-hard-sm">
          <div className="grid grid-cols-5 gap-4 p-4 text-xs font-bold uppercase tracking-wider text-muted-foreground border-b-2 border-foreground bg-muted/50">
            <div>Invoice #</div>
            <div>Client</div>
            <div>Amount</div>
            <div>Status</div>
          ...
          {invoices.map((invoice) => (
            <Link
              ...
              className="grid grid-cols-5 gap-4 p-4 text-sm hover:bg-tertiary/5 transition-colors border-b-2 border-foreground last:border-0 font-medium"
            >
